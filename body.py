class Body:
    def __init__(self, x=0, y=0, R=0, max_x=0, max_y=0):
        self.__R  = R
        self.__x1 = x
        self.__y1 = y
        self.__max_x = max_x
        ...
        if self.__draw and self.__draw_line:
            if self.__R:
                if not self.__visible:
                    self.__visible = True
                    self.__draw(self.__x1, self.__y1, self.__R)
                    self.__draw_line(self.__x1, self.__y1, self.__x2, self.__y2)

    def hide(self):
        if self.__hide and self.__hide_line:
            if self.__visible:
                self.__visible = False
                self.__hide(self.__x1, self.__y1, self.__R)
                self.__hide_line(self.__x1, self.__y1, self.__x2, self.__y2)

    def rotate(self, x, y):
        if self.__draw_line and self.__hide_line and self.__visible:
            self.hide()
            self.calc_x2_pos(x, y)
            self.show()
